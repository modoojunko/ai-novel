...
机械转写自 client/frontend/src/data/genres.ts（去掉前端专用 icon 字段），
camelCase 键名与前端 GenreDefinition 一致，供 genres/service.ensure_seed_genres()
在启动时幂等播种。用户对预置题材的编辑（如改名）不会被 seed 覆盖。
"""

PRESET_GENRES: list[dict] = [
    # ── 都市系 ──────────────────────────────────────────────────────────
    {
        "id": "urban-daily",
        "name": "都市日常",
    ...
    },
    {
        ...
        "promptInjection": "[末世废土基调] 生存是第一准则，资源匮乏贯穿始终，人性在极端环境下的各种写照",
        "toneBlueprint": {
            "defaultTone": "冷峻荒凉",
