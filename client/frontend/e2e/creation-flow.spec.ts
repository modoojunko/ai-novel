...
test.beforeAll(async () => {
  const s = await sRegisterAndLogin();
  token = s.token;
  username = s.username;
  restoreSession = writeOAuthSession(token, username);
});

test.afterAll(() => restoreSession());
...
test("简介空不可完成设定，保存后可确认（AC-4.2）", async ({ page }) => {
  const pid = await createNovel(page, "简介确认");
...
test("EmptyState 旁路：设定未完成可继续创作（AC-4.3）", async ({ page, request }) => {
  const pid = await createNovel(page, "旁路测试");
...
test("改名：顶栏就地改名即时生效（AC-2.x）", async ({ page }) => {
  const pid = await createNovel(page, "原始名字");
  const nextName = `新名字${Date.now() % 10000}`;
  ...
  // 旧名不再显示
  await expect(page.getByText("原始名字").first()).not.toBeVisible();
});
