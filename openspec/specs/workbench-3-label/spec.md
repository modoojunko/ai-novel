...
- 章纲 SHALL render `OutlineEditor` fed by `useOutline` (chapter data loaded on demand via `loadChapterData`).
- 提示词 SHALL render `PromptManagementPage` filtered to the currently selected chapter (`chapterRef` prop).
...
- `PromptManagementPage` SHALL accept an optional `chapterRef` prop; when provided, the overview SHALL render only that chapter's card (auto-expanded) and only fetch prompt metadata for it.
...
- Then the sub-label bar renders 正文 / 章纲 / 提示词
- When the user opens 提示词
- Then `PromptManagementPage` shows only the currently selected chapter's prompts, with a generate button and editable prompt content
