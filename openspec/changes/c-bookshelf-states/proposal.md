...
- **回访态（2026-08-29 二次裁定）**：不设独立继续创作条——书架按修改时间倒排即天然「继续」入口，卡片自携状态；顶部横条属冗余
- **首启态**：书架为空时渲染三步引导空态（新建作品 → 配置模型 → 开写第一章），替代现有单薄空态
- **满额态**：免费额度墙——顶部 `.notice.info` 说明 + 「新建作品/导入」按钮**锁定可见可点**（点击引导升级而非隐藏）+ 网格尾「升级锁卡」
...
（无——满额墙行为是对既有 design-system「Free vs PRO gating stays visible」Requirement 的合规落地（锁定可见而非隐藏），无 Requirement 文本变化，`.openspec.yaml` 已设 `skip_specs: true`）
