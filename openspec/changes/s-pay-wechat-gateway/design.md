...
| create_refund | POST /v3/refund/domestic/refunds | **请求带 notify_url 指向回调端点——退款回调以此开通，漏传则只剩 T3 查询兜底**；SUCCESS/PROCESSING→**受理**（受理≠成功，结果以查询/回调推进）；带 wx_refund_id。错误分类：`NOT_ENOUGH`（账户余额不足，不自愈）→转人工告警、T3 不自动重试空转；`FREQUENCY_LIMITED`（受理中）与 `ORDER_NOT_READY`（订单处理中）→按官方建议**原退款单号**间隔重试（勿换单号）；`USER_ACCOUNT_ABNORMAL`（用户账号注销）→转人工告警；其余资金类错误同转告警 |
| query_refund | GET /v3/refund/domestic/refunds/{no} | 状态归一到 RefundStatus（SUCCESS/PROCESSING/CLOSED/ABNORMAL 四态全覆盖）。**ABNORMAL（V2 CHANGE）为终态异常**：原路退卡失败（卡作废/冻结），官方指定处置=商户平台交易中心人工处理——告警转人工，不自动重试；退款到账时效（零钱 ≤20min、银行卡最长 3 个工作日）决定 T3 轮询须耐心衰减勿提前判死；**查无此退款单 → 受理丢失信号，告警**（勿当 PROCESSING 干等） |
...
- **回调重试风暴**：微信 15s/15s/30s/3m/10m/20m/30m/30m/30m/60m/3h/3h/3h/6h/6h 共 15 次衰减重试；所有路径按官方 v3 应答规范止重试（成功 200/204 无报文，失败 4xx/5xx+FAIL body）。**多台服务器可能几秒内并发重发同一通知**（官方明示）——CAS 状态迁移+幂等键即官方要求的「数据锁防函数重入」，契约测试须含并发重放场景
