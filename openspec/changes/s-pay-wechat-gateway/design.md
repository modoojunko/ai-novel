...
| close_payment | POST …/close | **先查单确认未付再关单**（官方建议）；关单成功→success；**微信返回 403 `ORDERPAID`（订单已支付，无法关闭）→ already_paid=True**（官方单边账指引：遇 ORDERPAID 按支付成功处理）。注意 RESOURCE_EXISTS/ORDER_CLOSED 不是关单接口的错误码（前者不存在，后者属下单接口） |
