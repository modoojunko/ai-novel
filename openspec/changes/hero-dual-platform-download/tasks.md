...
- [ ] 4.1 开 PR 到 main（只圈 HeroSection、版本常量、workflow、本 change 工件；不夹带工作区既有未提交文件），CI 绿后合入。验证：PR Files changed 清单核对
- [ ] 4.2 合入后线上抽查：S端 自动部署完成后，用云端 webReader 抓 `www` 落地页确认双按钮、版本化直链与版本副行上线；gh api 核对按钮目标资产在 v0.1 Release 上真实存在。验证：抓取输出含「下载 macOS 版」与 `v0.1`；gh api 资产列表匹配
