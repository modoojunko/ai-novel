...
## 改进点 · 启动主页重构（2026-08-28 立项，独立推进）

**裁定**：桌面 App 双击打开的第一屏是 home，不是营销页——`/` 由 LandingPage 改为书架（`/novels`）；App 内 Landing 整体摘除，营销内容唯一住在 S端 官网/下载页；书架「升级」入口直连 S端 portal。这条裁定同时是 P0 #4 的正解范本：额度墙用「锁定可见 + 一句话说明 + 升级出口」，不再隐藏入口。

**设计稿**：[`home.html`](./home.html)（右下角评审切换器为非基线元素，parity 截图排除）：

| 态 | 内容 |
| --- | --- |
| 回访 | 继续创作条（上次编辑的书置顶直达工作台）+ 启动健康检查（Key 未配置等，阻断型在前）+ 书架网格 |
| 首启 | 三步引导空态（新建作品 → 配置模型 → 开写第一章）——是任务路径，不是 hero，无任何营销段落 |
| 书架满额 | 免费额度墙：新建按钮锁定可见 + notice 一句话说明 + 网格尾升级锁卡 |

词汇纪律：全稿与 `design/{base,list}.css` 逐字同源，图标只用注册表键；唯一新增组件 `.resume`（继续创作条），落地时先 ADJUSTMENTS 登记再进 base.css 共享段（双端同批）。实施 5 步与回归口径见 [`handoff.md`](./handoff.md) 六、独立改进点小节。
