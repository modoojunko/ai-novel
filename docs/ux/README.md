...
| [`home.html`](./home.html) | **启动主页设计稿（2026-08-28 立项，2026-08-29 落地收官）**：/ 路由裁定 v2（免登录静态首页 + 已登录跳书架）、三态原型（回访 / 首启 / 书架满额）、实施清单与验收口径；`.resume` 继续创作条上线当天即裁撤（#238：书架按修改时间倒排即「继续」入口） |
| [`uikit/`](./uikit/) | 可搬运的 React 候选组件集 + CSS 补丁，搬运顺序见其 [README](./uikit/README.md) |

五份 HTML 可直接双击打开阅读；`components.html` 内含可交互演示（确认弹窗、Toast、撤销窗口），`home.html` 右下角可切换三个状态。

## 本目录是设计标准的唯一权威（2026-08-27 起）

分工三层，各居其所，冲突时以靠前者为准：

- **标准＝本目录**：规范正文 `design-language.html`、全端一致性裁决 `cross-end.html`、候选实现 `uikit/`。它派生出唯一的机器强制层——两端 `scripts/design-vocab.mjs`（禁令同源，改动与标准同批）。语义、词汇、状态语言、组件档位的最终裁决都在这里。
- **原型基线＝`../design-c/prototypes/`**：像素与交互基线 `*.html`、**层内规范 `prototypes/CLAUDE.md`**（token 逐字值 / 组件类尺寸 / 页面清单 / 避坑——只对原型层负责，其原「全站唯一权威」声明已改为指向本目录）、比对产物 `baselines/`、登记簿 `prototypes/ADJUSTMENTS.md`。这些是被 `design:check` 消费的资产，不承载标准。
- 原型层与本目录冲突时，以本目录为准，原型侧修订必须登记进 `ADJUSTMENTS.md`（先例：2026-08-29 六份规范块 token 对齐 + CLAUDE.md 权威声明分层化）。
...
### P0 · 功能断裂（✅ 2026-08-29 全部关账：#1/#2/#5/#6 随 #226 止血批，#3/#4 随 #237 书架三态）
...
- 提示词页签对免费改为「锁定可见」，兑现「同一界面」承诺（✅ 2026-08-29 用户拍板：可见但不能用；现行代码与 ai-prompt-crafting spec 是「免费隐藏」，落地时须同批改 spec）
...
## 改进点 · 启动首页与书架改版（2026-08-28 立项，2026-08-29 裁定 v2）

**裁定 v2（用户拍板，推翻 v1）**：`/` 必须是**免登录可见的静态页**——v1「`/` 即书架、Landing 摘除」作废；现状 LandingPage 保留位置并按新设计语言重做，已登录用户打开 `/` 自动跳 `/novels`（不多看一眼营销内容）。书架侧不变：这条裁定同时是 P0 #4 的正解范本——额度墙用「锁定可见 + 一句话说明 + 升级出口」，不再隐藏入口。

**设计稿**：[`home.html`](./home.html)（右下角评审切换器为非基线元素，parity 截图排除；默认态 = 静态首页）：

| 态 | 路由 | 内容 |
| --- | --- | --- |
| home 静态首页 | `/`（未登录） | 登录/建号入口卡 + 一句实用信息（数据只存本机）——用户已下载安装，不做产品介绍 |
| 回访 | `/novels` | 书架网格（按修改时间倒排，卡片自携「继续创作」入口——.resume 顶部横条已裁撤）+ 启动健康检查（阻断型在前） |
| 首启 | `/novels` | 三步引导空态（新建作品 → 配置模型 → 开写第一章）——是任务路径，不是 hero |
| 书架满额 | `/novels` | 免费额度墙：新建按钮锁定可见 + notice 一句话说明 + 网格尾升级锁卡 |

词汇纪律：全稿与 `design/{base,list}.css` 逐字同源，图标只用注册表键；新增组件 `.welcome`（静态首页入口卡）与 `.first-run/.lock-tile`（书架空态/锁卡），落地时先 ADJUSTMENTS 登记再进 base.css 共享段（双端同批）。~~`.resume` 继续创作条~~ 2026-08-29 二次裁定裁撤（#238），勿再实现。实施 5 步与回归口径见 [`handoff.md`](./handoff.md) 六、独立改进点小节。
