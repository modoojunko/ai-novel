...
| [`home.html`](./home.html) | **启动主页设计稿（2026-08-28 立项）**：/ 路由裁定（书架即 home、App 内 Landing 摘除）、三态原型（回访 / 首启 / 书架满额）、继续创作条新组件、实施清单与验收口径 |
...
| 回访 | `/novels` | 继续创作条（上次编辑的书置顶直达工作台）+ 启动健康检查（阻断型在前）+ 书架网格 |
| 首启 | `/novels` | 三步引导空态（新建作品 → 配置模型 → 开写第一章）——是任务路径，不是 hero |
| 书架满额 | `/novels` | 免费额度墙：新建按钮锁定可见 + notice 一句话说明 + 网格尾升级锁卡 |

词汇纪律：全稿与 `design/{base,list}.css` 逐字同源，图标只用注册表键；新增组件 `.resume`（继续创作条）与 `.welcome`（静态首页入口卡）两个，落地时先 ADJUSTMENTS 登记再进 base.css 共享段（双端同批）。实施 5 步与回归口径见 [`handoff.md`](./handoff.md) 六、独立改进点小节。
