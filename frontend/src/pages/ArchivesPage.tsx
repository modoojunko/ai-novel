...
export default function ArchivesPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-120px)]">
      ...
      {/* Right: Reading area */}
      <div className="flex-1 overflow-y-auto relative bg-[#1a1f2e]">
        <div
        ...
        {selectedFilename && currentInfo ? (
          <div className="relative z-10 max-w-[640px] mx-auto py-9 px-8">
            <div className="text-center mb-10">
              <span className="text-[10px] text-base-content/60 tracking-[2px] uppercase">
                第{currentInfo.vol}卷 · 第{currentInfo.ch}章
              </span>
              <h2 className="mt-3 mb-2 font-serif text-2xl">{currentInfo.title}</h2>
            </div>

            <div className="font-serif text-[16px] leading-[2]">
              {content ? (
                content.split("\n\n").map((para, i) => (
                  <p key={i} className="indent-[2em] mb-[0.6em]">{para}</p>
                ))
              ) : (
                <p className="text-base-content/30 italic indent-0">加载中...</p>
              )}
            </div>

            <div className="flex items-center justify-between mt-12 pt-5 border-t border-base-300">
              <span
                className={`text-[11px] cursor-pointer transition-colors ${
                  prevFile ? "text-base-content/60 hover:text-primary" : "text-base-content/20 cursor-default"
                }`}
              ...
              <span
                className={`text-[11px] cursor-pointer transition-colors ${
                  nextFile ? "text-primary hover:text-primary/80" : "text-base-content/20 cursor-default"
                }`}
